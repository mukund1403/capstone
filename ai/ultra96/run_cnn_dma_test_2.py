...
def _apply_preprocess(x: np.ndarray, preprocess: str) -> np.ndarray:
    if preprocess == "none":
...
def _normalize_window(x: np.ndarray, norm_stats):
    ...
    x2 = _apply_preprocess(x2, preprocess)
    x2 = (x2 - mean) / std
...
def _normalize_input(input_data: Any) -> np.ndarray:
    """
    Convert incoming sensor payload to a flat float32 array of length 450.
    Accepts:
      - list/np.ndarray shape (75,6) or length 450
