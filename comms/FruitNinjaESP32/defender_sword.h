#ifndef DEFENDER_SWORD_H
#define DEFENDER_SWORD_H
#include <Wire.h>
...
void IRAM_ATTR onPauseButton() {
  ...
  if (nowUs - btnLastIsrUs < 20000) return;
  btnLastIsrUs = nowUs;
  btnIsrReq = true;
}
...
// LOOP
// ============================================================
void defenderSwordLoop(PublishFn publishFn, PublishPauseFn pauseFn) {
  unsigned long now = millis();
